...
[[ -z $SOURCED && ( ${+terminfo} = 1 && -n ${terminfo[colors]} ) || ( ${+termcap} = 1 && -n ${termcap[Co]} ) ]] && {
    ZINIT+=(
        ...
        col-url    $'\e[38;5;45m'
        col-meta   $'\e[38;5;57m'
        col-meta2  $'\e[38;5;32m'
        col-data   $'\e[38;5;82m'
        col-data2  $'\e[38;5;50m'
        col-hi     $'\e[38;5;184m'
        col-ehi    $'\e[01m\e[31m'
        col-var    $'\e[38;5;63m'
        col-glob   $'\e[38;5;226m'
        col-cmd    $'\e[38;5;90m'
        col-nl     $'\n'
    )
}
...
.zinit-load-snippet() {
    ...
    if [[ -z ${opts[(r)--command]} && ( -z ${ZINIT_ICE[as]} || ${ZINIT_ICE[as]} = null ) ]]; then
        ...
        } else { [[ ${+ZINIT_ICE[silent]} -eq 1 || ${+ZINIT_ICE[pick]} -eq 1 && -z ${ZINIT_ICE[pick]} || ${ZINIT_ICE[pick]} = /dev/null ]] || { +zinit-message "Snippet not loaded ({info2}${id_as}{rst})"; retval=1; } }
...
+zinit-message() {
    ...
    local msg=${(j: :)${(@)${@//(#b)([\[]([^\]]##)[\]])/${ZINIT[col-$match[2]]-$match[1]}}//(#b)([\{]([^\}]##)[\}])/${ZINIT[col-$match[2]]-$match[1]}}}
...
# !atpull-post
@zinit-register-hook "ZINIT_ICE[reset]" hook:e-\!atpull-post ∞zinit-reset-hook
