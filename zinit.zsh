...
.zinit-ice() {
    ...
    builtin setopt extendedglob noksharrays noshortloops typesetsilent warncreateglobal
    builtin zmodload zsh/zutil 2>/dev/null || return 1

    local opt_help
    local -a cli_usage=('Usage: zinit ice [-h|--help] [args ...]')
    zparseopts -D -F -K -- {h,-help}=opt_help || return 1
    if [[ -n "$opt_help" ]]; then
        builtin print -l -- $cli_usage
        local -a ice_order
        ice_order=( ${${(s.|.)ZINIT[ice-list]}:#teleid} ${(@)${(@)${(@Akons:|:u)${ZINIT_EXTS[ice-mods]//\'\'/}}/(#s)<->-/}:#(.*|dynamic-unscope)} )
        +zi-log "{nl}Available ice-modifiers:"
        builtin print -a -C 4 -x 2 -- ${ice_order[*]}
        return 0
    fi
