...
typeset -gAH ZINIT ZINIT_COMPDEF_REPLAY ZINIT_CUR_BIND_MAP ZINIT_EXTS \
  ZINIT_EXTS2 ZINIT_ICES ZINIT_REPORTS ZINIT_SICE ZINIT_SNIPPETS
...
if [[ ! -o functionargzero || ${options[posixargzero]} = on || ${ZINIT[ZERO]} != */* ]]; then
  ZINIT[ZERO]="${(%):-%N}"
fi

: ${ZINIT[BIN_DIR]:="${ZINIT[ZERO]:h}"}

if [[ ${ZINIT[BIN_DIR]} = \~* ]]; then
...
ZINIT[cmds]="-h|--help|help|man|self-update|times|doctor|load|light|unload|snippet|ls|ice|\
...
ZINIT[PLUGINS_DIR]=${~ZINIT[PLUGINS_DIR]}
ZINIT[COMPLETIONS_DIR]=${~ZINIT[COMPLETIONS_DIR]}
ZINIT[SNIPPETS_DIR]=${~ZINIT[SNIPPETS_DIR]}
ZINIT[SERVICES_DIR]=${~ZINIT[SERVICES_DIR]}
...
if [[ -z ${path[(re)$ZPFX/bin]} && -d "$ZPFX/bin" ]]; then
  path=( "$ZPFX/bin" "${path[@]}" )
elif [[ -z ${path[(re)$ZPFX/sbin]} && -d "$ZPFX/sbin" ]]; then
  path=( "$ZPFX/sbin" "${path[@]}" )
fi

# Add completions directory to fpath.
if [[ -z ${fpath[(re)${ZINIT[COMPLETIONS_DIR]}]} ]]; then
  fpath=( "${ZINIT[COMPLETIONS_DIR]}" "${fpath[@]}" )
elif [[ ! -d $ZSH_CACHE_DIR ]]; then
  command mkdir -p "$ZSH_CACHE_DIR"
elif [[ -n ${ZINIT[ZCOMPDUMP_PATH]} ]]; then
  ZINIT[ZCOMPDUMP_PATH]=${~ZINIT[ZCOMPDUMP_PATH]}
fi
...
ZINIT[NEW_AUTOLOAD]=0
if is-at-least 5.1; then
  ZINIT[NEW_AUTOLOAD]=1
fi
...
if ! zmodload zsh/zutil; then
  builtin print -P "%F{196}zsh/zutil module is required, aborting Zinit set up.%f"
  return 1
elif ! zmodload zsh/parameter; then
  builtin print -P "%F{196}zsh/parameter module is required, aborting Zinit set up.%f"
  return 1
fi
...
if [[ -z $SOURCED ]] && [[ ${+terminfo} -eq 1 && -n ${terminfo[colors]} ]] || [[ ${+termcap} -eq 1 && -n ${termcap[Co]} ]]; then
  ZINIT+=(
    # Basic colors
    col-error   $'\e[1m\e[38;5;204m' col-p       $'\e[38;5;81m'      col-pname $'\e[1;4m\e[32m'
    col-failure $'\e[38;5;204m'      col-happy   $'\e[1m\e[38;5;82m' col-annex $'\e[38;5;153m'
    col-id-as   $'\e[4;38;5;220m'    col-version $'\e[3;38;5;87m'    col-info  $'\e[38;5;82m'
    col-info2   $'\e[38;5;227m'      col-profile $'\e[38;5;148m'     col-note  $'\e[38;5;148m'
    col-info3   $'\e[1m\e[38;5;227m' col-slight  $'\e[38;5;230m'     col-uname $'\e[1;4m\e[35m'
    col-keyword $'\e[32m'            col-uninst  $'\e[38;5;118m'
    # Modern colors:
    col-apo    $'\e[1;38;5;45m'  col-aps   $'\e[38;5;117m'   col-bapo   $'\e[1;38;5;220m'
    col-baps   $'\e[1;38;5;82m'  col-bar   $'\e[38;5;82m'    col-th-bar $'\e[38;5;82m'
    col-bcmd   $'\e[38;5;220m'   col-bspc  $'\b'             col-b-warn $'\e[1;38;5;214m'
    ...
    col-u-warn $'\e[4;38;5;214m' col-cmd   $'\e[38;5;82m'    col-ice    $'\e[38;5;39m'
  ...
  if [[ ( ${+terminfo} -eq 1 && ${terminfo[colors]} -ge 256 ) || ( ${+termcap} -eq 1 && ${termcap[Co]} -ge 256 ) ]]; then
    ZINIT+=(
      col-pname $'\e[1;4m\e[38;5;39m'
      col-uname  $'\e[1;4m\e[38;5;207m'
    )
  fi
fi
    ...
    if (( ${+functions[$func]} != 1 )); then
      ...
      if [[ ${ZINIT[NEW_AUTOLOAD]} = 2 ]]; then
        ...
        if (( ${+opts[(r)-C]} )); then
          ...
          eval "function ${(q)func} { local -a fpath fpath=( ${(qqq)PLUGIN_DIR} ${(qqq@)fpath_elements} ${(qqq@)fpath} ) builtin autoload -X ${(j: :)${(q-)opts[@]}} }"
          retval=$?
        fi
      else
        eval "function ${(q)func} { :zinit-reload-and-run ${(qqq)PLUGIN_DIR}"$'\0'"${(pj,\0,)${(qqq)fpath_elements[@]}} ${(qq)opts[*]} ${(q)func} "'"$@" }'
        retval=$?
      fi
      if (( ZINIT[ALIASES_OPT] )); then
...
:zinit-tmp-subst-zstyle() {
  builtin setopt extendedglob localoptions noerrexit noerrreturn noshortloops nowarncreateglobal typesetsilent unset
...
:zinit-tmp-subst-alias() {
  builtin setopt extendedglob localoptions noerrexit noerrreturn noshortloops typesetsilent unset warncreateglobal
  ...
  for a in "$@"; do
    ...
    if (( ${+aliases[$aname]} )); then
      .zinit-add-report "${ZINIT[CUR_USPL2]}" "Warning: redefining alias \`${aname}', previous value: ${aliases[$aname]}"
    fi
    ...
    if [[ -n ${ZINIT[CUR_USPL2]} ]]; then
      ZINIT[ALIASES__${ZINIT[CUR_USPL2]}]+="$quoted "
    fi
    # Remember for dtrace.
    if [[ ${ZINIT[DTRACE]} = 1 ]]; then
      ZINIT[ALIASES___dtrace/_dtrace]+="$quoted "
    fi
  done
...
:zinit-tmp-subst-zle() {
  ...
  if [[ ( $1 = -N && ( $# = 2 || $# = 3 ) ) || ( $1 = -C && $# = 4 ) ]]; then
    ...
    if [[ ${ZINIT_ZLE_HOOKS_LIST[$2]} = 1 ]]; then
      ...
      if [[ -n ${ZINIT[CUR_USPL2]} ]]; then
        ZINIT[WIDGETS_DELETE__${ZINIT[CUR_USPL2]}]+="$quoted "
      fi
      # Remember for dtrace.
      if [[ ${ZINIT[DTRACE]} = 1 ]]; then
        ZINIT[WIDGETS_DELETE___dtrace/_dtrace]+="$quoted "
      fi
      ...
      # Remember only when load is in progress (it can be dstart that leads execution here).
      if [[ -n ${ZINIT[CUR_USPL2]} ]]; then
        ZINIT[WIDGETS_SAVED__${ZINIT[CUR_USPL2]}]+="$quoted "
      fi
      # Remember for dtrace.
      if [[ ${ZINIT[DTRACE]} = 1 ]]; then
        ZINIT[WIDGETS_SAVED___dtrace/_dtrace]+="$quoted "
      fi
...
.zinit-tmp-subst-on() {
  ...
  #
  # One could expect possibilities of widening of temporary substituting of
  # functions, however such sequence doesn't exist, e.g. "light" then
  # "load"/"dtrace", "compdef" then "load"/ "dtrace", "light" then "compdef",
  # "compdef" then "light".
  #
  # It is always "dtrace" then "load" (i.e. dtrace then load) "dtrace" then
  # "light" (i.e.: dtrace then light load) "dtrace" then "compdef" (i.e.:
  # dtrace then snippet).
...
.zinit-diff-functions() {
  ...
  if [[ $cmd = begin ]]; then
    if [[ -z ${ZINIT[FUNCTIONS_BEFORE__$uspl2]} ]]; then
...
.zinit-diff-options() {
  local IFS=" "
  if [[ $2 = begin &&  -z ${ZINIT[OPTIONS_BEFORE__$uspl2]} ]]; then
...
.zinit-diff-env() {
  ...
  if [[ $2 = begin ]]; then
    if [[ -z ${ZINIT[PATH_BEFORE__$uspl2]} ]]; then
      tmp=( "${(q)path[@]}" )
      ZINIT[PATH_BEFORE__$1]="${tmp[*]}"
    elif [[ -z ${ZINIT[FPATH_BEFORE__$uspl2]} ]]; then
      tmp=( "${(q)fpath[@]}" )
...
.zinit-diff-parameter() {
  typeset -a tmp
  if [[ $2 = begin && -z ${ZINIT[PARAMETERS_BEFORE__$uspl2]} ]]; then
...
.zinit-any-to-pid() {
  builtin emulate -LR zsh ${=${options[xtrace]:#off}:+-o xtrace}
  builtin setopt extendedglob noshortloops rcquotes typesetsilent ${${${+REPLY}:#0}:+warncreateglobal}
...
.zinit-get-object-path() {
  ...
  for type ( ${=${${(M)type:#AUTO}:+snippet plugin}:-$type} ) {
    ...
    if [[ -e $local_dir/${dirname:+$dirname/}._zinit || -e $local_dir/${dirname:+$dirname/}._zplugin ]];then
      exists=1
      (( exists )) && break
    fi
  }
...
@zinit-substitute() {
  builtin emulate -LR zsh ${=${options[xtrace]:#off}:+-o xtrace}
  builtin setopt extendedglob noshortloops typesetsilent warncreateglobal
...
.zinit-compdef-clear() {
  ...
  if [[ ! $quiet = -q ]]; then
    +zinit-message "Compdef-replay cleared (it had {num}${count}{rst} entries)"
  fi
} # ]]]

# FUNCTION: .zinit-compdef-replay. [[[
# Runs gathered compdef calls  and enables running compinit after loading plugins
.zinit-compdef-replay() {
  ...
  if [[ ${+functions[compdef]} = 0 ]]; then
    +zinit-message "{u-warn}Error{b-warn}:{rst} The {func}compinit{rst} function was not loaded, unable do {it}{cmd}compdef replay{rst}"
    return 1
  fi
  ...
  for cdf in "${ZINIT_COMPDEF_REPLAY[@]}"; do
    ...
    pos=( "${(Q)pos[@]}" )
    if [[ ! $quiet = -q ]]; then
      +zinit-message "Running compdef: {cmd}${pos[*]}{rst}"
    fi
    compdef "${pos[@]}"
  done
...
.zinit-load-plugin() {
  ...
  if [[ ${ICE[as]} = command ]]; then
    ...
    if [[ -n ${ICE[src]} || -n ${ICE[multisrc]} || ${ICE[atload][1]} = "!" ]]; then
      if (( ZINIT[TMP_SUBST] == 0 )); then
      ZINIT[TMP_SUBST]=inactive; builtin setopt noaliases; (( ${+ZINIT[bkp-compdef]} )) && functions[compdef]="${ZINIT[bkp-compdef]}" || unfunction compdef; (( ZINIT[ALIASES_OPT] )) && builtin setopt aliases; }
      fi
    fi
    ...
    if [[ $ICE[atinit] = '!'* ]]; then
      { local ___oldcd="$PWD"; (( ${+ICE[nocd]} == 0 )) && { () { setopt localoptions noautopushd; builtin cd -q "${${${(M)___user:#%}:+$___plugin}:-${ZINIT[PLUGINS_DIR]}/${___id_as//\//---}}"; } && eval "${ICE[atinit]#!}"; ((1)); } || eval "${ICE[atinit]#1}"; () { setopt localoptions noautopushd; builtin cd -q "$___oldcd"; }; }
    fi
    if (( ${+ICE[silent]} )); then
      {
        {
          [[ -n $___precm ]] && { builtin ${___precm[@]} 'source "$ZERO"'; ((1)); } \
            || { ((1)); $___builtin source "$ZERO"; };
        } 2>/dev/null 1>&2; (( ___retval += $? ));
        ((1));
      } || \
        {
        ((1));
        {
          [[ -n $___precm ]] \
            && { builtin ${___precm[@]} 'source "$ZERO"'; ((1)); } \
            || { ((1)); $___builtin source "$ZERO"; };
        };
        (( ___retval += $? ));
      }
...
.zinit-load-snippet() {
  ...
  if [[ $url = *(${(~kj.|.)${(Mk)ZINIT_1MAP:#OMZ*}}|robbyrussell*oh-my-zsh|ohmyzsh/ohmyzsh)* ]]; then
  ...
  if (( ${+ICE[atinit]} )); then
    if local ___oldcd="$PWD"; (( ${+ICE[nocd]} == 0 )); then
      () {
  ...
  if [[ -z ${opts[(r)--command]} && ( -z ${ICE[as]} || ${ICE[as]} = null || ${+ICE[null]} -eq 1 || ${+ICE[binary]} -eq 1 ) ]]; then
    ...
    elif ! [[ ${+ICE[silent]} -eq 1 || ${+ICE[pick]} -eq 1 && -z ${ICE[pick]} || ${ICE[pick]} = /dev/null ]]; then
    ...
    if (( ${+ICE[svn]} )); then
      ...
      # This makes no sense, but users may come up with something.
      [[ -n ${ICE[pick]} ]] && { list=( ${(M)~ICE[pick]##/*}(DN) $local_dir/$dirname/${~ICE[pick]}(DN) ) [[ -n ${list[1-correct]} ]] && xpath="${list[1-correct]:h}" xfilepath="${list[1-correct]}" }
    fi
    ...
    # Run the function wrapping & tracking requests
    if [[ -n ${ICE[wrap]} ]]; then
      (( ${+functions[.zinit-service]} )) || builtin source "${ZINIT[BIN_DIR]}/zinit-additional.zsh"
      .zinit-wrap-functions "$save_url" "" "$id_as"
    fi
...
.zinit-prepare-home() {
  ...
  elif [[ ! -d ${ZINIT[PLUGINS_DIR]}/_local---zinit ]]; then
  ...
  elif [[ ! -d ${ZINIT[COMPLETIONS_DIR]} ]]; then
  ...
  elif [[ ! -d ${ZINIT[SNIPPETS_DIR]} ]]; then
  ...
  elif [[ ! -d ${~ZINIT[MAN_DIR]}/man9 ]]; then
...
#   local i in=$1 match out rest spaces wrk
...
.zinit-formatter-pid() {
  ...
  # Replace at least one character with an unbreakable space, because due to
  # implementation problems, marginal whitespace is lost ...
  pbz=${pbz/[[:blank:]]/ }
...
.zinit-ice() {
  ...
  for bit; do
    if [[ $bit = (#b)(--|)(${~ZINIT[ice-list]}${~exts})(*) ]]; then
...
# Remembers all ice-mods, assigns them to concrete plugin. Ice spec is in
# general forgotten for second-next command (that's why it's called "ice" - it
# melts), however they glue to the object (plugin or snippet) mentioned in the
# next command – for later use with e.g. `zinit update ...'.
.zinit-pack-ice() {
...
# $4 - task index in the ZINIT[WAIT_ICE_...] fields
# $5 - mode: load or light
# $6 - the plugin-spec or snippet URL or alias name (i.e., id-as ice)
.zinit-run-task() {
  ...
  elif [[ -n ${ICE[subscribe]#\!} && -n $(( ___s=0 )) && $___pass = 3 ]]; then
    local -a fts_arr
    if eval "fts_arr=( ${ICE[subscribe]}(DNms-$(( EPOCHSECONDS - ZINIT[fts-${ICE[subscribe]}] )))); (( \${#fts_arr} ))" && {ZINIT[fts-${ICE[subscribe]}]="$EPOCHSECONDS"; ___s=${+ICE[once]};}; || (( 0 )); then
...
# Searches for timeout tasks executes them. A array of tasks waiting for
# execution, this scheduler manages them, detects which ones should be run at
# current moment, decides to remove (or not) them from the array after
# execution.
#
...
#      this is handy if e.g. a docker image starts up and needs to install all
#      turbo-mode plugins without any hesitation (delay), i.e. "burst" allows
#      to run package installations from script, not from prompt
@zinit-scheduler() {
  ...
  if [[ -n $1 ]]; then
    if [[ ${#ZINIT_RUN} -le 1 || $1 = following ]]; then
      () {
        ...
        #
        # This either will not change ZINIT_TASKS entry - when ___i is
        # used in the ternary expression, or replaces an entry with
        # <no-data> (i.e., ZINIT_TASKS[1] entry)
        ...
        for (( ___idx1 = 0; ___idx1 <= 4; ___idx1 ++ )) {
          for (( ___idx2 = 1; ___idx2 <= (___idx >= 4 ? 1 : 3); ___idx2 ++ )) {
            # The following substitution could be this:
            # However, Zsh <= 5.3.1 makes the use of the period (,) impossible inside ${..//$arr[$(( ...))]}
            #
            ...
            ZINIT_TASKS=( ${ZINIT_TASKS[@]/(#b)([0-9]##)+([0-9]##)+([1-3])(*)/${ZINIT_TASKS[$(( (___ar2=${match[2]}+1) ? ( (___ar3=${(M)match[3]%[1-3]}) ? ( (___ar4=___idx1+1) ? ( (___ar5=___idx2) ? ( (${match[1]}+${match[2]}) <= $___t ? zinit_scheduler_add(___i++) : ___i++ ) : 1 ) : 1 ) : 1 ) : 1  ))]}} )
    ...
    # There is a bug in Zsh: first sched call would not be issued until a
    # key-press, if sched +1 ... would be called inside zle -F handler
    # Due to this, it is called in precmd-handle code
    ...
    command true # workaround bug - https://www.zsh.org/mla/workers/2018/msg00966.html
    ...
    *)
      ...
      if (( ${#reply} )); then
        ...
        if (( ${+functions[${reply[5]}]} )); then
          ...
          +zinit-message "({error}Couldnt find the subcommand-handler {obj}${reply[5]}{error} of the z-annex {file}${reply[3]}{error})"
          return 0
        fi
      fi
...
if zmodload zsh/datetime {
  add-zsh-hook -- precmd @zinit-scheduler
...
builtin alias zi=zinit zini=zinit zpl=zinit zplg=zinit
...
if [[ -e ${${ZINIT[BIN_DIR]}}/module/COMPILED_AT ]]; then
  local compiled_at_ts="$(<${${ZINIT[BIN_DIR]}}/module/COMPILED_AT)"
fi
if [[ -e ${${ZINIT[BIN_DIR]}}/module/RECOMPILE_REQUEST ]]; then
  local recompile_request_ts="$(<${${ZINIT[BIN_DIR]}}/module/RECOMPILE_REQUEST)"
fi
