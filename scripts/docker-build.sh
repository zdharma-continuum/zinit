...
build() {
  ...
  if docker build --build-arg "PUSERNAME=$(id -u -n)" --build-arg "PUID=$(id -u)" --build-arg "PGID=$(id -g)" --build-arg "TERM=${TERM:-xterm-256color}" --build-arg "ZINIT_ZSH_VERSION=${zsh_version}" --file "$dockerfile" --tag "${image_name}:${tag}" "${args[@]}" "$(realpath ..)"; then
    {
