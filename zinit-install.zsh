...
if ! builtin source "${ZINIT[BIN_DIR]}/zinit-side.zsh"; then
...
.zinit-jq-check() {
  if (( ${+commands[curl]} ));then
    ...
    +zinit-message "{error}ERROR{rst}: {cmd}jq{msg2} binary not found{rst}"
    return 1
  fi
} # ]]]
...
.zinit-json-to-array() {
  ...
  # NOTE We're not using @sh for the keys on purpose. Associative array keys
  # are used verbatim by zsh: typeset -A a; a[key]=one; a['key']=two; echo
  # ${(k)a} 'key' key
...
.zinit-get-package() {
  ...
  local user=$1 pkg=$2 plugin=$2 id_as=$3 dir=$4 profile=$5 ver=${ICE[ver]}
  local local_path=${ZINIT[PLUGINS_DIR]}/${3//\//---} pkgjson tmpfile=${$(mktemp):-${TMPDIR:-/tmp}/zsh.xYzAbc123}
  ...
  elif ! [[ .zinit-download-file-stdout $URL 0 1 2>/dev/null > $tmpfile ]]  ; then
    # retry
  ...
  for required ( $requirements ) {
    ...
    if [[ -n ${ICE[dl]} && -z ${(k)ZINIT_EXTS[(r)<-> z-annex-data: zinit-annex-patch-dl *]} ]]; then
    ...
    if [[ -n ${message} ]]; then
      +zinit-message "{info}${message}{rst}"
    fi
    ...
    if (( !${+ICE[git]} && !${+ICE[from]} )); then
      ...
      if ! .zinit-download-file-stdout "$URL" 0 1 >! "$fname" ; then
        if ! .zinit-download-file-stdout "$URL" 1 1 >! "$fname"; then
          command rm -f "$fname"
          +zinit-message "Download of the file {apo}{file}$fname{apo}{rst} failed."
          +zinit-message "Install one of the following: {cmd}${(pj:$tool_sep:)${=:-curl wget lftp lynx}}{rst}."
          return 1
        fi
      fi
    fi
...
.zinit-setup-plugin-dir() {
  builtin emulate -LR zsh ${=${options[xtrace]:#off}:+-o xtrace}
  setopt extendedglob noshortloops rcquotes warncreateglobal

  local id_as=$3 local_path plugin=$2 remote_url_path=${1:+$1/}$2 tpe=$4 update=$5 user=$1 version=$6
  ...
  if [[ $user = _local ]]; then
    +zinit-message "{u-warn}WARNING{b-warn}:{error} no local plugin {pid}$plugin{error} located at {cmd}$local_path{rst}"
    return 1
  fi

  command rm -f ${TMPDIR:-/tmp}/zinit.{'execs','installed_comps','skipped_comps','compiled'}.$$.lst
  ...
  (
    if [[ $site = */releases ]]; then
      ...
      if .zinit-get-latest-gh-r-url-part "$user" "$plugin" "$url"; then
        return $?
      fi
...
.zinit-install-completions() {
  ...
  .zinit-download-file-stdout() {
    ...
    if (( restart ));then
      (( ${path[(I)/usr/local/bin]} )) || { path+=( "/usr/local/bin" ); trap "path[-1]=()" EXIT }
      ...
      if (( ${+commands[curl]} )); then
        ...
        +zinit-message "{u-warn}ERROR{b-warn}:{rst}No download tool detected (i.e., {cmd}curl{rst}, {cmd}wget{rst}, {cmd}lftp{rst}, {cmd}lynx{rst})."
        return 2
      fi
    else
  ...
  .zinit-get-url-mtime() {
    ...
    if ! LANG=C TZ=UTC strftime -r -s REPLY "%d %b %Y %H:%M:%S GMT" "$header" &>/dev/null;then
      REPLY=$(( $(date +"%s") ))
      return 4
    fi
  ...
  .zinit-mirror-using-svn() {
    ...
    if ! (( ${+commands[svn]} ));then
      builtin print -Pr -- "${ZINIT[col-error]}Warning:%f%b Subversion not found" ", please install it to use \`${ZINIT[col-obj]}svn%f%b' ice."
    fi
  ...
  .zinit-forget-completion() {
    ...
    if [[ "${#commands}" -gt 0 ]] && (( quiet == 0 ));then
      +zinit-message "{msg2}Forgetting commands completed by {obj}${f}{rst}"
    fi
    ...
    for k ( $commands ) {
      unset "_comps[$k]"
      if ! (( quiet ));then
        builtin print -Prn "${${first:#1}:+, }${ZINIT[col-info]}$k%f%b"
      fi
      first=0
    }
    if ! (( quiet || first ));then
  ...
  (
    ...
    .zinit-update-snippet() {
      ...
      if (( ${+ICE[svn]} )); then
        ...
        url=${url/(#s)(#m)(${(~kj.|.)ZINIT_2MAP})/$ZINIT_2MAP[$MATCH]}
        if [[ $save_url == (${(~kj.|.)${(Mk)ZINIT_1MAP:#OMZ*}})* ]];then
        ...
        elif [[ $save_url = (${(~kj.|.)${(kM)ZINIT_1MAP:#PZT*}})* ]]; then
          if [[ $url != *.zsh ]];then
            url+=/init.zsh
          fi
        fi
    ...
    .zinit-get-latest-gh-r-url-part() {
      ...
      if ${+commands[musl-gcc]} || find /lib/ -maxdepth 1 -name '*musl*' >/dev/null 2>&1; then
        HAS_MUSL='linux-musl'
      fi
      ...
      for bpick ( "${bpicks[@]}" ) {
        ...
        if (( $#list > 1 && ${+commands[anbox]} == 1 ));then
        ...
        if (( $#list > 1 && ${+commands[dpkg-deb]} == 1 ));then
    ...
    ziextract() {
      ...
      zparseopts -D -E -move=opt_move -move2=opt_move2 -norm=opt_norm  -auto=opt_auto -nobkp=opt_nobkp \
        || { +zinit-message "{error}ziextract:{msg2} Incorrect options given to {pre}ziextract{msg2} (available are: {meta}--auto{msg2}, {meta}--move{msg2}, {meta}--move2{msg2}, {meta}--norm{msg2}, {meta}--nobkp{msg2}).{rst}"; return 1; }

      local file="$1" ext="$2"
      integer auto=${${${(M)${#opt_auto}:#0}:+0}:-1} move2=${${${(M)${#opt_move2}:#0}:+0}:-1} move=${${${(M)${#opt_move}:#0}:+0}:-1} nobkp=${${${(M)${#opt_nobkp}:#0}:+0}:-1} norm=${${${(M)${#opt_norm}:#0}:+0}:-1}
        ...
        if (( !${#files} ));then
          ...
          for file ( $archives ) {
            ...
            if [[ $type = (zip|rar|xz|7-zip|gzip|bzip2|tar|exe|pe32) ]];then
              if (( !OPTS[opt_-q,--quiet] ));then
                +zinit-message "{pre}ziextract:{info2} Note:{rst} detected a {meta}$type{rst} archive in the file {file}$fname{rst}."
              fi
              ...
              if [[ -f $fname || -f ${fname:r} ]];then
                ...
                for file2 ( $archives2 ) {
                  ...
                  if [[ $type != $type2 && $type2 = (zip|rar|xz|7-zip|gzip|bzip2|tar) ]]; then
                    ...
                    if (( !OPTS[opt_-q,--quiet] ));then
                      +zinit-message "{pre}ziextract:{info2} Note:{rst} detected a {obj}${type2}{rst} archive in the file {file}${fname}{rst}."
                    fi
      ...
      →zinit-check() {
        if ! (( ${+commands[$1]} )); then
          +zinit-message "{error}ziextract:{msg2} Error:{msg} No command {data}$1{msg}, it is required to unpack {file}$2{rst}."
        fi
        ...
        case "${${ext:+.$ext}:-$file}" in
            ...
            ;;
            ((#i)*.exe|(#i)*.pe32)
            →zinit-extract() { command chmod a+x -- ./$file ./$file /S /D="`cygpath -w $PWD`"; }
            ;;
        esac
