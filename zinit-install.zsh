...
.zinit-get-latest-gh-r-url-part() {
    ...
    matchstr=(
      ...
      darwin '*((#s)|/)*(darwin|apple|mac(os|))*((#e)|/)*'
      ...
      x86_64 "((386|686|x86_64|amd*(#e))~*(rm|arch|)*)"
    )
    # x86_64 "((linux(_amd)?64|x86_64|intel|amd64|macos)#e)~*(a(rm|rch)64*)"
      # darwin '(darwin|apple|(mac(os)?))'
    ...
    for bpick ( "${bpicks[@]}" ) {
        ...
        if (( $#list > 1 )) {
            ...
            list2=( ${list[@]:#(#i)*${~matchstr[${MACHTYPE}]}*} )
            print -C 1 ${(pj:\n:)${list[@]:t}}
            (( $#list2 > 0 )) && list=( ${list2[@]} )
        }
