...
_zi::get-package() {
    ...
    } elif { ! _zi::download-file-stdout $URL 0 1 2>/dev/null > $tmpfile } {
        # retry
        command rm -f $tmpfile
        _zi::download-file-stdout $URL 1 1 2>/dev/null >1 $tmpfile
    }
    ...
    if [[ -z $pkgjson ]] {
        +zi::message "{error}❌ Error: the package {hi}${pkgname}" \
    ...
    local -a profiles
    profiles=("${(@f)$(_zi::json-get-value "$pkgjson" "${json_ices} | keys[]")}") \
        || return 1
    ...
    local -A metadata
    _zi::json-to-array "$pkgjson" "$json_meta" metadata
    if [[ "$?" -ne 0 || -z "$metadata" ]] {
        +zi::message '{error}❌ ERROR: Failed to retrieve metadata from package.json'
        return 1
    }
    ...
    _zi::json-to-array "$pkgjson" "$json_profile" ICE
    ...
    +zi::message "{info3}Package{ehi}:{rst} ${pkgname}. Selected" \
...
"{profile}${(pj:$pro_sep:)${profiles[@]:#$profile}}{rst}."
    ...
    [[ -n ${message} ]] && +zi::message "{info}${message}{rst}"
    ...
    if (( !${+ICE[git]} && !${+ICE[from]} )) {
        (
            local -A jsondata
            local URL=$(_zi::json-get-value "$pkgjson" "_resolved")
            local fname="${${URL%%\?*}:t}"
            ...
            if { ! _zi::download-file-stdout "$URL" 0 1 >! "$fname" } {
                if { ! _zi::download-file-stdout "$URL" 1 1 >! "$fname" } {
                    command rm -f "$fname"
...
_zi::setup-plugin-dir() {
    ...
    if _zi::get-object-path plugin "$id_as" && [[ -z $update ]] {
    ...
    (
        if [[ $site = */releases ]] {
            ...
            if [[ -z $tag_version ]]; then
                tag_version="$( { _zi::download-file-stdout $site/latest || _zi::download-file-stdout $site/latest 1; } 2>/dev/null | \
                                  command grep -m1 -o 'href=./'$user'/'$plugin'/releases/tag/[^"]\+')"
                tag_version=${tag_version##*/}
            fi
            local url=$site/expanded_assets/$tag_version

            _zi::get-latest-gh-r-url-part "$user" "$plugin" "$url" || return $?
            ...
            (
                ...
                for REPLY ( $reply ) {
                    ...
                    if { ! _zi::download-file-stdout "$url" 0 1 >! "${REPLY:t}" } {
                        if { ! _zi::download-file-stdout "$url" 1 1 >! "${REPLY:t}" } {
                    ...
                    if _zi::download-file-stdout "$url.sig" 2>/dev/null >! "${REPLY:t}.sig"; then
                        :
                    else
            ...
            (
                () { setopt localoptions noautopushd; builtin cd -q "$local_path"; } || return 1
                _zi::get-cygwin-package "$remote_url_path" || return 1
            ...
            case ${ICE[proto]} in
                (|https|git|http|ftp|ftps|rsync|ssh)
                    ...
                    :zi::git-clone |& { command ${ZINIT[BIN_DIR]}/share/git-process-output.zsh || cat; }
                    if (( pipestatus[1] == 141 )) {
                        :zi::git-clone
                        integer retval=$?
                        if (( retval )) {
        ...
        if [[ $update != -u ]] {
            ...
            _zi::store-ices "$local_path/._zinit" ICE "" "" "" ""
            reply=(
    ...
    [[ (0 = ${+ICE[nocompletions]} && ${ICE[as]} != null && ${+ICE[null]} -eq 0) || 0 != ${+ICE[completions]} ]] && \
        _zi::install-completions "$id_as" "" "0"
...
# FUNCTION: _zi::install-completions [[[
...
_zi::install-completions() {
    ...
    _zi::exists-physically-message "$id_as" "" || return 1
    ...
    for c in "${completions[@]}"; do
        ...
        if [[ ( -z ${already_symlinked[(r)*/$cfile]} || $reinstall = 1 ) &&
            ...
            _zi::forget-completion "$cfile" "$quiet"
        else
            SKIPPED_COMPS+=( $cfile )
...
# FUNCTION: _zi::download-file-stdout [[[
...
_zi::download-file-stdout() {
    ...
    if (( restart )) {
        ...
        if (( ${+commands[curl]} )); then
            if [[ -n $progress ]]; then
                command curl --progress-bar -fSL "$url" 2> >(_zi::single-line >&2) || return 1
            else
        ...
        if type curl 2>/dev/null 1>&2; then
            ...
            _zi::download-file-stdout "$url" "1" "$progress"
            return $?
        fi
    }
...
# FUNCTION: _zi::forget-completion [[[
        ...
        if [[ -z ${ICE[(i)(\!|)(sh|bash|ksh|csh)]} ]] {
            () {
                ...
                if { ! zcompile -U "$first" } {
                    ...
                    +zi::message "{msg2}Warning:{rst} Consider submitting an error report to Zinit or to the plugin's author."
                } else {
...
_zi::download-snippet() {
            ...
            (
                ...
                if (( ${+ICE[svn]} )) {
                    if [[ $update = -u ]] {
                        ...
                        if ! _zi::mirror-using-svn "$url" "-t" "$dirname"; then
                        ...
                        if (( ZINIT[annex-multi-flag:pull-active] == 2 )) {
                            ...
                            _zi::mirror-using-svn "$url" "-u" "$dirname" || return 4
                        }
                    } else {
                        _zi::mirror-using-svn "$url" "" "$dirname" || return 4
                    }
                    ...
                    if (( !skip_dl )) {
                        if { ! _zi::download-file-stdout "$url" 0 1 >! "$dirname/$filename" } {
                            if { ! _zi::download-file-stdout "$url" 1 1 >! "$dirname/$filename" } {
    ...
    "${${(M)OPTS[opt_-q,--quiet]:#1}:+, skip the -q/--quiet option for more information}.{rst}"; retval=4; }
                ...
                if (( !OPTS[opt_-q,--quiet] )) && [[ $url != /dev/null ]] {
                    +zi::message "{msg}Linking {file}$filename{msg}{…}{rst}"
                    command ln -svf "$url" "$local_dir/$dirname/$filename" || \
                        { +zi::message "{ehi}ERROR:{error} The file linking has been unsuccessful.{rst}"; retval=4; }
                } else {
                    command ln -sf "$url" "$local_dir/$dirname/$filename" &>/dev/null || \
                        { +zi::message "{ehi}ERROR:{error} The link of {file}$filename{error} has been unsuccessful"\
    "${${(M)OPTS[opt_-q,--quiet]:#1}:+, skip the -q/--quiet option for more information}.{rst}"; retval=4; }
                }
            }
        }
        ...
        if [[ ${${:-$local_dir/$dirname}%%/##} != ${ZINIT[SNIPPETS_DIR]} ]] {
            ...
            _zi::store-ices "$pfx" ICE url_rsvd "" "$save_url" "${+ICE[svn]}"
        } elif [[ -n $id_as ]] {
    ...
    OPTS[opt_-q,--quiet]=1
    [[ (0 = ${+ICE[nocompletions]} && ${ICE[as]} != null && ${+ICE[null]} -eq 0) || 0 != ${+ICE[completions]} ]] && \
        _zi::install-completions "%" "$local_dir/$dirname" 0
...
_zi::update-snippet() {
    ...
    if { ! _zi::get-object-path snippet "$id_as" } {
    ...
    _zi::download-snippet "$save_url" "$url" "$id_as" "$local_dir" "$dirname" "$filename" "-u"

    return $?
} # ]]]
# FUNCTION: _zi::single-line [[[
...
# FUNCTION: _zi::get-latest-gh-r-url-part [[[
...
_zi::get-latest-gh-r-url-part() {
    ...
    if [[ -z $urlpart ]] {
        ...
        if [[ -z $tag_version ]]; then
            ...
            tag_version="$( { _zi::download-file-stdout $releases_url || _zi::download-file-stdout $releases_url 1; } 2>/dev/null | \
                              command grep -m1 -o 'href=./'$user'/'$plugin'/releases/tag/[^"]\+')"
            tag_version=${tag_version##*/}
        fi
    ...
    init_list=( ${(@f)"$( { _zi::download-file-stdout $url || _zi::download-file-stdout $url 1; } 2>/dev/null | \
...
ziextract() {
    ...
    case "${${ext:+.$ext}:-$file}" in
        ...
        ((#i)*.gz|(#i)*.gzip)
            ...
            →zinit-extract() {
                ...
                _zi::get-mtime-into "$file" 'ZINIT[tmp]'
        ...
        ((#i)*.dmg)
            →zinit-extract() {
                ...
                if (( retval )) {
                    +zi::message "{info}[{pre}ziextract{info}]{error} Error:{msg} problem occurred when attempted to copy the files" \
    ...
    if [[ $(typeset -f + →zinit-extract) == "→zinit-extract" ]] {
        _zi::extract-wrapper "$file" →zinit-extract || {
    ...
    unfunction -- _zi::extract-wrapper
...
# FUNCTION: _zi::get-cygwin-package [[[
_zi::get-cygwin-package() {
    ...
    local mlst="$(mktemp)"
    while (( retry -- )) {
        if ! _zi::download-file-stdout https://cygwin.com/mirrors.lst 0 > $mlst; then
            _zi::download-file-stdout https://cygwin.com/mirrors.lst 1 > $mlst
        fi
    ...
    retry=3
    while (( retry -- )) {
        if ! _zi::download-file-stdout ${mirror}x86_64/setup.bz2 0 1 > $setup.bz2; then
            _zi::download-file-stdout ${mirror}x86_64/setup.bz2 1 1 > $setup.bz2
        fi
    ...
    while (( retry -- )) {
        integer retval=0
        if ! _zi::download-file-stdout $url 0 1 > $outfile; then
            if ! _zi::download-file-stdout $url 1 1 > $outfile; then
...
# FUNCTION: _zi::configure-run-autoconf [[[
...
${${${(M)flags:#*\#*}:+$msg}:-$msg_}
        _zi::countdown aclocal,\ autoconf,\ automake && \
            (
                cd -q $dir
                rm -f aclocal.m4
                aclocal -I m4 --force
    ...
    if (( ( OPTS[opt_-r,--reset] && ZINIT[-r/--reset-opt-hook-has-been-run] == 0 ) || \
        ...
        if [[ $type == snippet ]] {
            ...
            if (( is_release && !skip_pull )) {
                if (( option )) {
                    (( !OPTS[opt_-q,--quiet] )) && +zi::message "{pre}reset ($msg_bit): {msg2}running: {rst}rm -rf ${${ZINIT[PLUGINS_DIR]:#[/[:space:]]##}:-${TMPDIR:-/tmp}/xyzabc312}/${${(M)${local_dir##${ZINIT[PLUGINS_DIR]}[/[:space:]]#}:#[^/]*}:-${TMPDIR:-/tmp}/xyzabc312-zinit-protection-triggered}/*"
                    ...
                    (( !OPTS[opt_-q,--quiet] )) && +zi::message "{pre}reset ($msg_bit): {msg2}running: {rst}${ICE[reset]:-rm -rf ${${ZINIT[PLUGINS_DIR]:#[/[:space:]]##}:-${TMPDIR:-/tmp}/xyzabc312}/${${(M)${local_dir##${ZINIT[PLUGINS_DIR]}[/[:space:]]#}:#[^/]*}:-${TMPDIR:-/tmp}/xyzabc312-zinit-protection-triggered}/*}"
...
~zi::configure-base-hook() {
    ...
    [[ $flags == *\#* || ! -f $dir/configure ]] &&
        _zi::configure-run-autoconf "$dir" $flags
...
~zi::atclone-hook() {
    ...
    local rc=0
    [[ -n $atclone ]] && _zi::countdown atclone && {
        local ___oldcd=$PWD
...
~zi::extract-hook() {
    ...
    _zi::extract plugin "$extract" "$dir"
} # ]]]
# FUNCTION: ~zi::mv-hook [[[
~zi::mv-hook() {
    [[ -z $ICE[mv] ]] && return 0
...
~zi::compile-plugin-hook() {
    ...
    if [[ -z $ICE[(i)(\!|)(sh|bash|ksh|csh)] ]] {
        () {
            ...
            if [[ $tpe == snippet ]] {
                _zi::compile-plugin "%$dir" ""
            } else {
                _zi::compile-plugin "$id_as" ""
            }
        }
    }
} # ]]]
...
~zi::atpull-e-hook() {
    ...
    _zi::countdown atpull && {
        local ___oldcd=$PWD
        (( ${+ICE[nocd]} == 0 )) && {
            () { setopt localoptions noautopushd; builtin cd -q "$dir"; }
        }
        _zi::at-eval "$atpull" "$ICE[atclone]"
        rc="$?"
        () { setopt localoptions noautopushd; builtin cd -q "$___oldcd"; };
    }
