...
clean:
	@zsh -fc "print -C4 *.zwc(N) */zsdoc/{$(foreach wrd,$(SRC),$(wrd).zsh.adoc)(N),data(/N)} tests/_support/tmp*(/N)"
	zsh -fc "rm -rf *.zwc(N) */zsdoc/{$(foreach wrd,$(SRC),$(wrd).zsh.adoc)(N),data(/N)} tests/_support/tmp*(/N)"
