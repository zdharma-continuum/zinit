...
| `svn`       | Download an entire subdirectory of a repository as a snippet, e.g. `zinit ice svn; zinit snippet OMZP::git`. Other ice `pick` can be used to select file to source (default are: `*.plugin.zsh`, `init.zsh`, `*.zsh-theme`). The ice name is historical – GitHub retired its Subversion bridge in January 2024, so this is now a blobless, depth-1 `git` checkout of the subdirectory, and Subversion is neither used nor required. Note that an update overwrites local modifications to tracked files. **Does not work with plugins.** |
...
| `atpull`     | Run command after updating (**only if new commits are waiting for download**), within plugin's directory. If starts with "!" then command will be ran before `mv` & `cp` ices and before `git pull`. Otherwise it is ran after them. Can be `atpull'%atclone'`, to repeat `atclone` Ice-mod.                                                                                                                                                                                                                                                                                                                                                              |
...
| `reset`      | Invokes `git reset --hard HEAD` for plugins and for `svn''` (subdirectory) snippets before pulling any new changes. This way `git` will not report conflicts if some changes were done in e.g.: `atclone''` ice. For file snippets and `gh-r` plugins it invokes `rm -rf *`.                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `status {plg-spec}`      | Git status for plugin or snippet. `--all` – do it for all plugins and snippets.                                                           |
...
| `update [-q] [-r] {plg-spec}\|URL\|--all`                        | Git update plugin or snippet.<br> `--all` – update all plugins and snippets.<br> `-q` – quiet.<br> `-r` \| `--reset` – run `git reset --hard` before pulling changes.                                                                                                                                                  |
