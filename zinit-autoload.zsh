...
# FUNCTION: _zi::unregister-plugin [[[
...
# FUNCTION: _zi::diff-functions-compute [[[
...
# FUNCTION: _zi::diff-options-compute [[[
...
# FUNCTION: _zi::diff-parameter-compute [[[
...
# FUNCTION: _zi::save-set-extendedglob [[[
...
# FUNCTION: _zi::get-completion-owner [[[
...
# FUNCTION: _zi::get-completion-owner-uspl2col [[[
...
_zi::get-completion-owner-uspl2col() {
    # "cpath" "readline_cmd"
    _zi::get-completion-owner "$1" "$2"
    _zi::any-colorify-as-uspl2 "$REPLY"
} # ]]]
# FUNCTION: _zi::find-completions-of-plugin [[[
...
# FUNCTION: _zi::check-comp-consistency [[[
...
# FUNCTION: _zi::check-which-completions-are-installed [[[
...
# FUNCTION: _zi::check-which-completions-are-enabled [[[
# For each argument that each should be a path to completion
# within a plugin's dir, it checks whether that completion
...
# FUNCTION: _zi::uninstall-completions [[[
...
_zi::uninstall-completions() {
    ...
    (( ${+functions[_zi::forget-completion]} )) || builtin source ${ZINIT[BIN_DIR]}"/zinit-install.zsh"
...
_zi::self-update() {
    ...
    (
        ...
        if (( ${#lines} > 0 )); then
            ...
            builtin print -rl -- "${lines[@]}" | _zi::pager
            builtin print
        fi
        if [[ $1 != -q ]] {
    ...
    for file ( "" -side -install -autoload ) {
        _zi::get-mtime-into "${ZINIT[BIN_DIR]}/zinit$file.zsh" "ZINIT[mtime$file]"
    }
} # ]]]
# FUNCTION: _zi::show-registered-plugins [[[
...
_zi::unload() {
    ...
    _zi::any-colorify-as-uspl2 "$uspl2"
    ...
    if [[ "$uspl2" != "_dtrace/_dtrace" ]]; then
        _zi::exists-message "$1" "$2" || return 1
    fi
    ...
    LASTREPORT=`_zi::show-report "$1" "$2"`
    ...
    # paranoid, don't want bad key/value pair error
    _zi::diff-options-compute "$uspl2"
    integer empty=0
    _zi::save-set-extendedglob
    [[ "${ZINIT[OPTIONS__$uspl2]}" != *[$'! \t']* ]] && empty=1
    _zi::restore-extendedglob
    ...
    # 6. Unfunction
    _zi::diff-functions-compute "$uspl2"
    typeset -a func
    func=( "${(z)ZINIT[FUNCTIONS__$uspl2]}" )
    local f
    for f in "${(on)func[@]}"; do
    ...
    _zi::diff-parameter-compute "$uspl2"
    empty=0
    _zi::save-set-extendedglob
...
_zi::show-report() {
    ...
    if [[ "$user/$plugin" != "_dtrace/_dtrace" ]]; then
        _zi::exists-message "$user" "$plugin" || return 1
    fi
...
# FUNCTION: _zi::show-debug-report [[[
# Displays dtrace report (data recorded in interactive session).
#
# User-action entry point.
_zi::show-debug-report() {
    _zi::show-report "_dtrace/_dtrace"
} # ]]]
# FUNCTION: _zi::update-or-status [[[
...
_zi::update-or-status() {
    ...
    _zi::set-m-func set
    trap "_zi::set-m-func unset" EXIT

    integer retval hook_rc was_snippet
    _zi::two-paths "$2${${2:#(%|/)*}:+${3:+/}}$3"
    if [[ -d ${reply[-4]} || -d ${reply[-2]} ]]; then
        _zi::update-or-status-snippet "$1" "$2${${2:#(%|/)*}:+${3:+/}}$3"
        retval=$?
        was_snippet=1
    fi

    _zi::any-to-user-plugin "$2" "$3"
    ...
    if (( was_snippet )) {
        _zi::exists-physically "$user" "$plugin" || return $retval
        _zi::any-colorify-as-uspl2 "$2" "$3"
        (( !OPTS[opt_-q,--quiet] )) && \
            +zi::message "{msg2}Updating also \`$REPLY{rst}{msg2}'" \
                "plugin (already updated a snippet of the same name){…}{rst}"
    } else {
        _zi::exists-physically-message "$user" "$plugin" || return 1
    }
    ...
    _zi::compute-ice "$user${${user:#(%|/)*}:+/}$plugin" "pack$nf" \
        ice local_dir filename is_snippet || return 1

    _zi::any-to-user-plugin ${ice[teleid]:-$id_as}
    ...
    if [[ -f $local_dir/.git/config ]]; then
        ...
        if [[ ${#${(M)config[@]:#\[remote[[:blank:]]*\]}} -eq 0 ]]; then
            (( !OPTS[opt_-q,--quiet] )) && {
                _zi::any-colorify-as-uspl2 "$id_as"
    ...
    if (( 1 )); then
        ...
        (( ${+functions[_zi::setup-plugin-dir]} )) || builtin source ${ZINIT[BIN_DIR]}"/zinit-install.zsh"
        if [[ $ice[from] == (gh-r|github-rel) ]] {
            {
                ICE=( "${(kv)ice[@]}" )
                _zi::get-latest-gh-r-url-part "$user" "$plugin" || return $?
            } always {
                ICE=()
            }
        } else {
        ...
        if (( 1 )) {
            if (( ZINIT[annex-multi-flag:pull-active] >= 1 )) {
                ...
                if (( ZINIT[annex-multi-flag:pull-active] >= 2 )) {
                    if ! _zi::setup-plugin-dir "$user" "$plugin" "$id_as" release -u $version; then
                        ZINIT[annex-multi-flag:pull-active]=0
                    fi
        ...
        _zi::store-ices "$local_dir/._zinit" ice "" "" "" ""
    fi
...
# FUNCTION: _zi::update-or-status-snippet [[[
#
...
_zi::update-or-status-snippet() {
    ...
    if [[ "$st" = "status" ]]; then
        ...
        (( ${+functions[_zi::setup-plugin-dir]} )) || builtin source ${ZINIT[BIN_DIR]}"/zinit-install.zsh"
        ICE=( "${(kv)ICE2[@]}" )
        _zi::update-snippet "${ICE2[teleid]:-$URL}"
        retval=$?
    fi
...
# FUNCTION: _zi::update-or-status-all [[[
...
_zi::update-or-status-all() {
    ...
    for file ( "" -side -install -autoload ) {
        _zi::get-mtime-into "${ZINIT[BIN_DIR]}/zinit$file.zsh" el; sum+=el
    }
    ...
    if [[ $2 != restart ]] && (( ZINIT[mtime] + ZINIT[mtime-side] +
        ...
        _zi::update-or-status-all "$1" restart
        return $?
    }
    ...
    if (( OPTS[opt_-s,--snippets] || !OPTS[opt_-l,--plugins] )) {
        ...
        for snip ( ${ZINIT[SNIPPETS_DIR]}/**/(._zinit|._zplugin)/mode(D) ) {
            ...
            [[ -f ${snip:h}/id-as ]] && \
                id_as="$(<${snip:h}/id-as)" || \
                id_as=
            _zi::update-or-status-snippet "$st" "${id_as:-$(<${snip:h}/url)}"
            ICE=()
        }
    ...
    for repo in ${ZINIT[PLUGINS_DIR]}/*; do
        ...
        if [[ $st = status ]]; then
            ...
            _zi::update-or-status update "$user" "$plugin"
            update_rc=$?
            [[ $update_rc -ne 0 ]] && {
...
# FUNCTION: _zi::update-all-parallel [[[
_zi::update-all-parallel() {
    builtin emulate -LR zsh ${=${options[xtrace]:#off}:+-o xtrace}
    ...
    if (( OPTS[opt_-s,--snippets] || !OPTS[opt_-l,--plugins] )) {
        for snip ( "${files[@]}" ) {
            ...
            _zi::update-or-status-snippet "$st" "$ef_id" &>! $PUFILE &

            PUAssocArray[$!]=$PUFILE

            _zi::wait-for-update-jobs snippets
        }
    }
    ...
    if (( OPTS[opt_-l,--plugins] || !OPTS[opt_-s,--snippets] )) {
        ...
        for repo ( "${files2[@]}" ) {
            ...
            _zi::any-colorify-as-uspl2 "$uspl"
            +zi::message "Updating $REPLY{…}" >! $PUFILE

            _zi::any-to-user-plugin "$uspl"
            local user=${reply[-2]} plugin=${reply[-1]}

            _zi::update-or-status update "$user" "$plugin" &>>! $PUFILE &

            PUAssocArray[$!]=$PUFILE

            _zi::wait-for-update-jobs plugins
...
# FUNCTION: _zi::wait-for-update-jobs [[[
...
_zi::show-times() {
    ...
    for entry in "${(@on)ZINIT[(I)TIME_[0-9]##_*]}"; do
        ...
        if [[ "$entry2" = (http|https|ftp|ftps|scp|${(~j.|.)${${(k)ZINIT_1MAP}%::}}):* ]]; then
            ...
            _zi::any-colorify-as-uspl2 "$user" "$plugin"
...
# FUNCTION: _zi::compile-uncompile-all [[[
...
_zi::compile-uncompile-all() {
    ...
    for p in "${plugins[@]}"; do
        ...
        if [[ "$compile" = "1" ]]; then
            _zi::compile-plugin "$user" "$plugin"
        else
            _zi::uncompile-plugin "$user" "$plugin" "1"
        fi
    done
} # ]]]
# FUNCTION: _zi::uncompile-plugin [[[
...
_zi::show-completions() {
    ...
    for cpath in "${completions[@]}"; do
        ...
        _zi::get-completion-owner "$cpath" "$rdlink"
    ...
    for c in "${packs[@]}"; do
        ...
        _zi::any-colorify-as-uspl2 "$unpacked[2]"
...
# FUNCTION: _zi::clear-completions [[[
...
# FUNCTION: _zi::search-completions [[[
...
_zi::search-completions() {
    ...
    for pp in "${plugin_paths[@]}"; do
        ...
        if [[ "${#completions[@]}" -gt 0 ]]; then
            ...
            _zi::any-colorify-as-uspl2 "${pp:t}"
...
_zi::cenable() {
    ...
    if [[ ! -e "$bkpfile" ]]; then
        ...
        _zi::check-comp-consistency "$cfile" "$bkpfile" 0
        return 1
    fi
    ...
    if [[ -e "$cfile" ]]; then
        ...
        _zi::check-comp-consistency "$cfile" "$bkpfile" 1
        command rm -f "$cfile"
    else
        _zi::check-comp-consistency "$cfile" "$bkpfile" 0
    fi
...
_zi::cdisable() {
    ...
    _zi::prepare-readlink
    # Get completion's owning plugin
    _zi::get-completion-owner-uspl2col "$bkpfile" "$REPLY"
...
# FUNCTION: _zi::run-delete-hooks [[[
_zi::run-delete-hooks() {
    if [[ -n ${ICE[atdelete]} ]]; then
        _zi::countdown "atdelete" && ( (( ${+ICE[nocd]} == 0 )) && \
...
_zi::delete() {
    ...
    if (( OPTS[opt_-a,--all] )); then
        _zi::confirm "Prune all plugins in \`${ZINIT[PLUGINS_DIR]}'"\
...
"${${ZINIT[SNIPPETS_DIR]%%[/[:space:]]##}:-${TMPDIR:-${TMPDIR:-/tmp}}/abcEFG312}/*~*/plugins(ND)"
    ...
    if (( OPTS[opt_-c,--clean] )) {
        _zi::confirm "Prune ${ZINIT[col-info]}CURRENTLY NOT LOADED${ZINIT[col-rst]}"\
...
loadedsnips=( \${\${ZINIT_SNIPPETS[@]% <*>}/(#m)*/\$(_zi::get-object-path snippet \"\$MATCH\" && builtin print -rn \$REPLY; )} )
...
final_todelete=( \${final_todelete[@]//(#m)*/\$( _zi::get-object-path snippet \"\${\${\${MATCH##\${dir}[/[:space:]]#}/(#i)(#b)(http(s|)|ftp(s|)|ssh|rsync)--/\${match[1]##--}://}//--//}\" && builtin print -r -- \$REPLY)} )
...
builtin print -Pr \"\$ZINIT[col-obj]Done (with the exit code: \$_retval).%f%b\""
    ...
    if (( is_snippet )); then
        if [[ "${+ICE2[svn]}" = "1" ]] {
            if [[ -e "$local_dir" ]]
            then
                _zi::confirm "Delete $local_dir? (it holds: ${(j:, :)${(@u)files}})" \
                    "_zi::run-delete-hooks snippet \"${ICE2[teleid]}\" \"\" \"$the_id\" \
                    \"$local_dir\"; \
        ...
        _zi::any-to-user-plugin "${ICE2[teleid]}"
        if [[ -e "$local_dir" ]]; then
            _zi::confirm "Delete $local_dir? (it holds: ${(j:, :)${(@u)files}})" \
                "_zi::run-delete-hooks plugin \"${reply[-2]}\" \"${reply[-1]}\" \"$the_id\" \
                \"$local_dir\"; \
                command rm -rf ${(q)${${local_dir:#[/[:space:]]##}:-${TMPDIR:-${TMPDIR:-/tmp}}/abcYZX321}}"
        else
...
_zi::recently() {
    ...
    for p in ${plugins[@]}; do
        ...
        if [[ -d .git ]]; then
            ...
            if [[ -n $gitout ]]; then
                _zi::any-colorify-as-uspl2 "$uspl1"
                builtin print -r -- "$REPLY"
            fi
        fi
        popd >/dev/null
...
_zi::create() {
    ...
    _zi::any-colorify-as-uspl2 "${${${(M)isorg:#(y|yes)}:+$org}:-$user}" "$plugin"
    local uspl2col="$REPLY"
    builtin print "Plugin is $uspl2col"

    if _zi::exists-physically "${${${(M)isorg:#(y|yes)}:+$org}:-$user}" "$plugin"; then
...
# FUNCTION: _zi::list-compdef-replay [[[
...
_zi::get-path() {
    ...
    _zi::get-object-path snippet "$id_as" || \
        _zi::get-object-path plugin "$id_as"
