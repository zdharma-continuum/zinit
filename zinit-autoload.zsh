...
__zi::self-update() {
    ...
    (
        ...
        && lines=( ${(f)"$(command git log --abbrev-commit --boundary --date=short --color --pretty=format:'%Cgreen%ad %Cred%h%Creset -%C(yellow)%d%Creset %s %Cblue[%cn]%Creset %Cblue%G?%Creset || %b' ..origin/HEAD)"} )
        if (( ${#lines} > 0 )); then
...
builtin print -Pr \"\$ZINIT[col-obj]Done (with the exit code: \$_retval).%f%b\""
    ...
    if (( is_snippet )); then
        if [[ "${+ICE2[svn]}" = "1" ]] {
            if [[ -e "$local_dir" ]]
            then
                __zi::confirm "Delete $local_dir? (it holds: ${(j:, :)${(@u)files}})" \
