...
__zi::self-update() {
    ...
    (
        ...
        && lines=( ${(f)"$(command git log --abbrev-commit --boundary --date=short --decorate --color --pretty=format:'%Cgreen%ad %Cred%h%Creset -%C(yellow)%d%Creset %s %Cblue[%cn]%Creset%n%N%n' ..origin/HEAD)"} )
        if (( ${#lines} > 0 )); then
