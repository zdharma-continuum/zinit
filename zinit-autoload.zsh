...
# Returns 0 or 1
-zinit-prompt(){
  read -q ${(%):-"?%s%F{cyan}zinit%f%s: ${1}? [y/N]: "} && REPLY=y
  print ''
...
.zinit-delete() {
    ...
    local o_verbose o_help o_all o_clean o_yes rc
    local usage=(
      'Usage:'
      ...
      '  -y, --yes     Don´t prompt user to confirm'
      ''
    )
    ...
    [[ -n "$o_help" ]] && { print -lr -- $usage; return 0 }
    (( $#o_clean && $#o_all )) && { builtin print -l -- "choose either --all or --clean"; return 0 }

    if (( $#o_all && !$#o_clean )); then
      local -a all_installed=(
        ${${ZINIT[COMPLETIONS_DIR]}%%[/[:space:]]##}/*~*_zinit(/ND)
      ...
      if (( $#o_yes )) || ( -zinit-prompt "delete all plugins and snippets (${#all_installed} total)" ); then
        ...
        command rm -rf -- **/*(-@); # delete any broken symlinks
        rc=$?
    ...
    for i in $@; do
      ...
      if [[ "$local_dir" != /* ]]; then
        +zinit-message "ERROR: $i is not a plugin or snippet"
        failed+=($i)
        continue
      fi
      local ICE2[teleid]="${ICE2[teleid]:-${ICE2[id-as]}}"
      local -a files
      files=(
        "$local_dir"/*(%DN:t) "$local_dir"/*(*DN:t)
        "$local_dir"/*(.DN:t) "$local_dir"/*(=DN:t)
        "$local_dir"/*(@DN:t) "$local_dir"/*(pDN:t)
        "$local_dir"/*.(zsh|sh|bash|ksh)(DN:t)
        "$local_dir"/*~*/.(_zinit|svn|git)(/DN:t)
      )
      (( !${#files} )) && files=( "no files?" )
      files=( ${(@)files[1,4]} ${files[4]+more…} )
      # make ices available for the hooks
      local -A ICE
      local ICE=( "${(kv)ICE2[@]}" )
      # delete snippet
      if (( is_snippet )); then
        if [[ "${+ICE2[svn]}" = "1" ]] {
          if [[ -e "$local_dir" ]]; then
            ...
            +zinit-message "error: $i is not a snippet"
            failed+=($i)
            continue
          fi
        } else {
      ...
      if [[ -e "$local_dir" ]]; then
        ...
        +zinit-message "ERROR: $i is not a plugin"
        failed+=($i)
        continue
      fi
        fi
    ...
    # if (( $#o_clean && !$#o_all )); then
    #   if ( -zinit-prompt "delete unloaded plugins and snippets" ); then
